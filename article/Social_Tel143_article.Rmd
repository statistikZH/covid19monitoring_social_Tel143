...
dat_by_month <- dat_by_month_raw %>% 
  ...
  ## Remove Merkmale der kategorie "Beratungsinhalt"
  filter(kategorie != "Medienerziehung/Swisscom") %>% 
  
  ## Remove Merkmale der kategorie Geschlecht 
  # filter(!kategorie %in% c("Divers", "nicht bestimmbar")) %>% 
  ...
  mutate(
    kategorie = case_when(
      kategorie == "Weiblich" ~ "Frauen",
      kategorie == "Männlich" ~ "Männer",
      TRUE ~ kategorie
    ) 
  )
...
dat_bm_total <- dat_by_month %>% 
  ...
  group_by(monat_label, jahr, variable, kategorie) %>% 
  summarise(
    sum_anzahl = sum(wert)
  ) 
...
dat_bm_diff_vorjahr_przt <- dat_bm_total %>% 
  group_by(jahr, variable, kategorie) %>% 
  summarise(
    sum_anzahl = sum(sum_anzahl)
  ) %>% 
  arrange(kategorie, jahr) %>% 
  group_by(variable, kategorie) %>%
  mutate(
...
dat_bm_diff_vorjahr_total <- dat_bm_total %>% 
  arrange(monat_label, kategorie, jahr) %>% 
  group_by(monat_label, variable, kategorie) %>%
  mutate(
...
dat_bq_diff_vorjahr <- dat_by_quartal %>%
  ...
  group_by(jahr, variable, kategorie, altersklasse) %>% 
  summarise(
    sum_anzahl = sum(wert)
  ) %>% 
  arrange(variable, kategorie, altersklasse, jahr) %>% 
  group_by(altersklasse, variable, kategorie) %>%
  mutate(
    diff_vorjahr_przt = (sum_anzahl / lag(sum_anzahl) - 1) * 100,
    diff_vorjahr_total = sum_anzahl - lag(sum_anzahl)
  ) 
...
para1_geschlecht <- dat_bm_total %>% 
  filter(variable == "Geschlecht") %>% 
  filter(!kategorie %in% c("nicht bestimmbar", "Divers")) %>% 
  #filter(jahr == 2020) %>% 
  group_by(jahr, kategorie) %>% 
  summarise(
    sum_anzahl = sum(sum_anzahl)
  ) %>% 
  spread(key = kategorie, value = sum_anzahl) %>% 
...
para1_Maenner <- filter(para1_geschlecht_zu_abhname, kategorie == "Männer")$diff_vorjahr_przt

## Frauen

para1_Frauen <- filter(para1_geschlecht_zu_abhname, kategorie == "Frauen")$diff_vorjahr_przt
...
para3_gespraechsdauer <- dat_bm_total %>% 
  filter(variable == "Gesprächsdauer") %>% 
  group_by(jahr, kategorie) %>% 
  mutate(minuten = sum_anzahl * 24 * 60) %>% 
  filter(kategorie == "Aritmetisches Mittel") %>% 
  summarise(
...
dat_bm_total_beratungsinhalte <- dat_bm_total %>% 
  filter(jahr == 2020) %>% 
  #filter(kategorie != "Sorge wegen Infektion") %>% 
  filter(variable == "Beratungsinhalt") %>% 
...
dat_bm_beratungsinhalt_przt2 <- dat_bm_total %>% 
  ...
  filter(variable == "Beratungsinhalt") %>%
  group_by(jahr, monat_label, variable, kategorie) %>% 
  summarise(
    sum_anzahl = sum(sum_anzahl)
  ) %>% 
  mutate(
...
readr::write_csv(p1, path = here::here("data/derived_data/dat_p1_anzahl_anrufe.csv"))

```

```{r, eval=FALSE}

## Prozent Anrufe Divers und nicht bestimmbar
...
p2 <- dat_by_quartal %>%
  ...
  mutate(altersklasse = factor(altersklasse, levels = aggregation_alter)) %>% 
  group_by(jahr, kategorie, altersklasse) %>% 
  summarise(
    sum_anzahl = sum(wert)
  )  
...
readr::write_csv(p2, path = here::here("data/derived_data/dat_p1_anzahl_anrufe_altersklasse.csv"))
...
Eindrücklich ist die Veränderung beim Thema «Suchtverhalten»: Die über 65-Jährigen riefen im ersten Halbjahr 2020 deswegen mehr als dreimal so häufig an wie im Vorjahr, während die Zahl der Anrufe bei den übrigen Alterskategorien stabil blieb. Ein möglicher Erklärungsansatz sind die Verhaltensempfehlungen der Behörden wegen der Pandemie. Angehörige der Risikogruppe, zu der Menschen im Pensionsalter gehören, wurden aufgefordert, zuhause zu bleiben und zwischenmenschliche Kontakte möglichst zu meiden. Diese «verordnete» Isolation dürfte bei vielen älteren Menschen ein Gefühl der Einsamkeit geweckt haben, dem sie wiederum mit Suchtmitteln zu begegnen versuchten.
...
beratungsinhalt_size_group <- dat_bm_total %>% 
  ...
  select(kategorie, size_group) 

beratungsinhalt_size_group$kategorie <- factor(beratungsinhalt_size_group$kategorie, levels = beratungsinhalt_size_group$kategorie)
...
df_helper <- tibble(
  ...
  monat_label = "Januar",
  kategorie = rep(beratungsinhalt_size_group$kategorie, 2),
  sum_anzahl = c(
...
p3 <- dat_bm_total %>% 
  ...
  filter(variable == "Beratungsinhalt") %>% 
  filter(kategorie != "Sorge wegen Infektion") %>%
  mutate(kategorie = factor(kategorie, levels = beratungsinhalt_size_group$kategorie)) 
...
p3 %>% 
  ...
  facet_wrap(~kategorie, scales = "free_y", ncol = 4) +
...
readr::write_csv(p3, path = here::here("data/derived_data/dat_p1_anzahl_anrufe_beratungsinhalt.csv"))
...
dat_by_month %>%
  ...
  ## filter for Schicht as this is the total count of calls
  filter(variable %in% c("Geschlecht", "Zusätzliche Beanspruchung")) %>% 
...
Die Daten und der Quellcode für diesen Artikel stehen öffenlich auf [GitHub](https://github.com/statistikZH/covid19monitoring_social_Tel143) zur Verfügung.
