...
unzip(zipfile = here::here("data/raw/2020-05-28/Kantonales_Amt_für_Statistik.zip"), exdir = here::here("data/raw/combined/"), junkpaths = TRUE)
...
files_del_vec <- list.files(path = here::here("data/raw/combined/"), pattern = "2019-1Q")
...
for (name in files_del_vec) {
  if (file.exists(paste0(here::here("data/raw/combined/"), "/", name))) {
    unlink(
      paste0(here::here("data/raw/combined/"), "/", name)
    )
  } else {
...
files_del_vec <- list.files(path = here::here("data/raw/combined/"), pattern = "2020-April")
...
files_copy_vec <- list.files(path = here::here("data/raw/2020-07-13/"), pattern = "Statistik")

for (name in files_copy_vec) {
  if (file.exists(paste0(here::here("data/raw/2020-07-13/"), "/", name))) {
    
    file.copy(
      from = paste0(here::here("data/raw/2020-07-13/"), "/", name),
      to = (paste0(here::here("data/raw/combined/"), "/", name))
    )
  } else {
...
path_20201204 <- str_c(here::here("data/raw"),"/", raw_date_20201204)
...
no_files <- length(list.files(path = here::here("data/raw/combined/")))

dat_in <- list()

for (i in seq(1:no_files)) {
  dat_in[[i]] <- readxl::read_excel(
    path = here::here("data/raw/combined/", list.files(path = here::here("data/raw/combined/"))[[i]]),
...
dat_filename <- tibble(
  ...
  file_name = list.files(path = here::here("data/raw/combined/"))
)
...
write_csv(dat_by_month_export, here::here("data/final/Social_Tel143_nach_monat.csv"))
write_csv(dat_by_quartal_export, here::here("data/final/Social_Tel143_nach_quartal.csv"))
...
aggregiert_alter <- c("bis 18", "19 bis 40", "41 bis 65", "über 65", "nicht bestimmbar")
aggregiert_kontakthaeufigkeit <- c("Erstkontakt", "gelegentlicher Kontakt", "regelmaessiger Kontakt")
